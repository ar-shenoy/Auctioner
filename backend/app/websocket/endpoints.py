...
from app.models import Auction, Match
...
async def websocket_match_endpoint(websocket: WebSocket, match_id: str, token: str) -> None:
    """WebSocket endpoint for match updates.
    
    Requires JWT token as query parameter. Sends match snapshots and event updates.
    """
    # Authenticate
    user_id = await _get_current_user_from_token(token)
    if not user_id:
    ...
    try:
        ...
        # Send initial snapshot of match
        async with AsyncSession(bind=None) as session:
            ...
            if match:
                snapshot = {
                    ...
                    "match_id": match.id,
                    "status": match.status,
                    "team_1_score": match.team_1_score or 0,
                    "team_2_score": match.team_2_score or 0,
