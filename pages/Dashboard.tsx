    ...
    return (
        <div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <StatCard title="Total Budget" value={`$${(totalBudget / 1000000).toFixed(2)}M`} icon={
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v.01" />
                    </svg>
                }/>
                 <StatCard title="Players per Team" value={(totalPlayers / (teams.length || 1)).toFixed(1)} icon={
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <ul className="space-y-3">
                        {players.slice(0, 5).map(player => (
                            <li key={player.id} className="flex justify-between items-center bg-gray-700/50 p-3 rounded-md">
                                ...
                                <span className="font-mono text-green-400">${player.basePrice.toLocaleString()}</span>
                            </li>
                        ))}
                    </ul>
                </div>
                <div className="bg-gray-800/50 p-6 rounded-xl border border-gray-700/50">
                    <h2 className="text-xl font-semibold mb-4">Team Budgets</h2>
                     <ul className="space-y-4">
                        {teams.slice(0, 5).map(team => (
                            <li key={team.id}>
                                <div className="flex justify-between mb-1">
                                    <span className="text-base font-medium text-white">{team.name}</span>
                                    <span className="text-sm font-medium text-gray-400">${team.budget.toLocaleString()}</span>
                                </div>
                                <div className="w-full bg-gray-700 rounded-full h-2.5">
                                    <div className="bg-blue-600 h-2.5 rounded-full" style={{ width: `${(team.budget / 2000000) * 100}%` }}></div>
                                </div>
                            </li>
                        ))}
                    </ul>
